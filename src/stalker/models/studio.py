...
class Studio(Entity, DateRangeMixin, WorkingHoursMixin):
    ...
    **Scheduling**
    ...
    last_scheduled_at = Column(GenericDateTime, doc="Stores the last schedule date")
    ...
    def __init__(
        self, daily_working_hours=None, now=None, timing_resolution=None, **kwargs
    ):
    ...
    def to_tjp(self) -> str:
        ...
        tjp = f'project {self.tjp_id} "{self.tjp_id}" {self.start.date()} - {self.end.date()} {{'
        tjp += f"\n{indent}timingresolution {self.timing_resolution.days * 86400 + self.timing_resolution.seconds // 60}min"
        tjp += f"\n{indent}now {now}"
        ...
        # working hours
        tjp += "\n"
        tjp += "\n".join(f"{indent}{l}" for l in self.working_hours.to_tjp.split("\n"))
...
class WorkingHours(Entity):
    ...
    def __init__(self, working_hours=None, daily_working_hours=None, **kwargs):
