...
def test_take_name_argument_is_skipped_defaults_to_default_value(
    setup_version_db_tests,
):
    """take_name arg is skipped take attr equals defaults.DEFAULT_VERSION_TAKE_NAME."""
    data = setup_version_db_tests
    data["kwargs"].pop("take_name")
    new_version = Version(**data["kwargs"])
    assert new_version.take_name == defaults.version_take_name


def test_take_name_argument_is_none(setup_version_db_tests):
    """TypeError raised if the take_name argument is None."""
    data = setup_version_db_tests
...
def test_take_name_attribute_is_none(setup_version_db_tests):
    """TypeError raised if the take_name attribute is set to None."""
    data = setup_version_db_tests
    with pytest.raises(TypeError) as cm:
...
def test_take_name_argument_is_empty_string(setup_version_db_tests):
    """ValueError raised if the take_name arg is given as an empty string."""
    data = setup_version_db_tests
    data["kwargs"]["take_name"] = ""
    with pytest.raises(ValueError) as cm:
        Version(**data["kwargs"])
    assert str(cm.value) == "Version.take_name can not be an empty string"


def test_take_name_attribute_is_empty_string(setup_version_db_tests):
    """ValueError raised if the take_name attribute is set to an empty string."""
    data = setup_version_db_tests
    with pytest.raises(ValueError) as cm:
        data["test_version"].take_name = ""
    assert str(cm.value) == "Version.take_name can not be an empty string"


def test_take_name_argument_is_not_a_string(setup_version_db_tests):
    """TypeError raised if the given take_name argument is not a string."""
    data = setup_version_db_tests
    test_values = [1, 1.2, ["a list"], {"a": "dict"}]
    for test_value in test_values:
        data["kwargs"]["take_name"] = test_value
        with pytest.raises(TypeError):
            Version(**data["kwargs"])


@pytest.mark.parametrize("test_value", [1, 1.2, ["a list"], {"a": "dict"}])
def test_take_name_attribute_is_not_a_string(test_value, setup_version_db_tests):
    """TypeError raised if the take_name attr is set to a value other than a str."""
    data = setup_version_db_tests
    with pytest.raises(TypeError):
        data["test_version"].take_name = test_value


def test_take_name_argument_is_formatted_to_empty_string(setup_version_db_tests):
    """ValueError raised if the take_name arg is formatted to an empty str."""
    data = setup_version_db_tests
    data["kwargs"]["take_name"] = "##$½#$"
    with pytest.raises(ValueError) as cm:
...
def test_take_name_attribute_is_formatted_to_empty_string(setup_version_db_tests):
    """ValueError raised if the take_name arg is formatted to an empty str."""
    data = setup_version_db_tests
    with pytest.raises(ValueError) as cm:
        data["test_version"].take_name = "##$½#$"
...
def test_take_name_argument_is_formatted_correctly(setup_version_db_tests):
    """take_name argument value is formatted correctly."""
    data = setup_version_db_tests
    for test_value in data["take_name_test_values"]:
        data["kwargs"]["take_name"] = test_value[0]
        new_version = Version(**data["kwargs"])
        assert new_version.take_name == test_value[1]


def test_take_name_attribute_is_formatted_correctly(setup_version_db_tests):
    """take_name attribute value is formatted correctly."""
    data = setup_version_db_tests
    for test_value in data["take_name_test_values"]:
        data["test_version"].take_name = test_value[0]
        assert data["test_version"].take_name == test_value[1]
...
def test_version_number_attribute_is_automatically_generated(setup_version_db_tests):
    ...
    assert data["test_version"].take_name == new_version.take_name
...
def test_parent_attribute_will_not_allow_circular_dependencies(setup_version_db_tests):
    ...
    assert (
        str(cm.value) == "<tp_SH001_Task1_TestTake_v001 (Version)> (Version) and "
...
def test_children_attribute_will_not_allow_circular_dependencies(
    ...
    assert (
        str(cm.value) == "<tp_SH001_Task1_TestTake_v003 (Version)> (Version) and "
        "<tp_SH001_Task1_TestTake_v002 (Version)> (Version) creates a "
        'circular dependency in their "children" attribute'
    )
...
def test_children_attribute_will_not_allow_deeper_circular_dependencies(
    ...
    assert (
        str(cm.value) == "<tp_SH001_Task1_TestTake_v004 (Version)> (Version) and "
        "<tp_SH001_Task1_TestTake_v002 (Version)> (Version) creates a "
        'circular dependency in their "children" attribute'
    )
...
def test_nice_name_attribute_is_working_properly(setup_version_db_tests):
    ...
    version1 = Version(task=task3, take_name="Take1@Main")
    DBSession.add(version1)
    DBSession.commit()
