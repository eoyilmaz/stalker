...
class ProjectTestDBCase(UnitTestDBBase):
    ...
    def setUp(self):
        ...
        self.test_repo2 = Repository(name="Commercials Repository 2", code="CR2")

        self.test_client = Client(name="Test Company", users=[self.test_user_client])
        DBSession.save(
            [
                self.test_image_format,
                self.test_project_type,
                self.test_project_type2,
                self.test_structure_type1,
                self.test_structure_type2,
    ...
    def test_fps_argument_is_given_as_non_float_or_integer_2(self):
        ...
        assert (
            str(cm.value) == "Project.fps should be a positive float or int, not list"
        )
    ...
    def test_fps_argument_is_negative(self):
        ...
        assert (
            str(cm.value) == "Project.fps should be a positive float or int, not -1.0"
        )
    ...
    def test_repositories_argument_is_not_a_list_of_repository_instances(self):
        ...
        """
        self.kwargs["repositories"] = ["not", 1, "list", "of", Repository, "instances"]
        with pytest.raises(TypeError) as cm:
            Project(**self.kwargs)
    ...
    def test_users_argument_is_working_properly(self):
        ...
        assert sorted(self.kwargs["users"], key=lambda x: x.name) == sorted(
    ...
    def test_users_attribute_is_working_properly(self):
        ...
        assert sorted(users, key=lambda x: x.name) == sorted(
            self.test_project.users, key=lambda x: x.name
        )
    ...
    @pytest.mark.data_file_name("project_to_tjp_output.jinja2")
    def test_to_tjp_is_working_properly(self, get_data_file):
...
class ProjectTicketsTestDBCase(UnitTestDBBase):
    ...
    def setUp(self):
        ...
        self.test_ticket1 = Ticket(project=self.test_project)
        ...
        self.test_ticket2 = Ticket(project=self.test_project)
        DBSession.add(self.test_ticket2)
        DBSession.commit()

        # create a new ticket and close and then reopen it
        self.test_ticket3 = Ticket(project=self.test_project)
        ...
        # create a new ticket and leave it open
        self.test_ticket4 = Ticket(project=self.test_project)
        DBSession.add(self.test_ticket4)
        DBSession.commit()

        # create a new Ticket and close it
        self.test_ticket5 = Ticket(project=self.test_project)
        DBSession.add(self.test_ticket5)
        self.test_ticket5.resolve()
        DBSession.commit()

        # create a new Ticket and close it
        self.test_ticket6 = Ticket(project=self.test_project)
        ...
        # create a new ticket and close it
        self.test_ticket7 = Ticket(project=self.test_project)
        DBSession.add(self.test_ticket7)
        self.test_ticket7.resolve()
        DBSession.commit()

        # create a new ticket and close it
        self.test_ticket8 = Ticket(project=self.test_project)
        ...
        # create a new ticket and close it
        self.test_ticket9 = Ticket(project=self.test_project)
    ...
    def test_tickets_attribute_returns_all_tickets_in_this_project(self):
        ...
        assert sorted(self.test_project.tickets, key=lambda x: x.name) == sorted(
            [
                self.test_ticket1,
                self.test_ticket2,
                self.test_ticket3,
    ...
    def test_open_tickets_attribute_returns_all_open_tickets_owned_by_this_user(self):
        ...
        # now check for exact items
        assert sorted(self.test_project.open_tickets, key=lambda x: x.name) == sorted(
            [
                self.test_ticket1,
                self.test_ticket2,
                self.test_ticket3,
