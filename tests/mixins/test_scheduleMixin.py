...
class ScheduleMixinTester(unittest.TestCase):
    ...
    def test_start_date_argument_is_not_a_date_object(self):
        ...
        for test_value in test_values:
            ...
            self.assertEqual(new_foo_obj.start_date,
                             new_foo_obj.end_date - new_foo_obj.duration)

            self.assertEqual(new_foo_obj.end_date, self.kwargs["end_date"])
    ...
    def test_start_date_attribute_is_not_a_date_object(self):
        ...
        for test_value in test_values:
            end_date = self.mock_foo_obj.end_date
            ...
            self.assertEqual(
                ...
                self.mock_foo_obj.end_date - self.mock_foo_obj.duration
            )

            # check if we still have the same end_date
            self.assertEqual(self.mock_foo_obj.end_date, end_date)
    ...
    def test_end_date_argument_is_not_a_date_object(self):
        """testing if default values will be for the end_date attribute used
        ...
        for test_value in test_values:
            self.kwargs["end_date"] = test_value
            new_foo_obj = SchedMixFooMixedInClass(**self.kwargs)

            self.assertEqual(new_foo_obj.end_date,
                             new_foo_obj.start_date + new_foo_obj.duration)

    def test_end_date_attribute_is_not_a_date_object(self):
        """testing if default values will be used for the end_date attribute
        when trying to set the end_date attribute to something other than a
        ...
        for test_value in test_values:
            self.mock_foo_obj.end_date = test_value

            self.assertEqual(self.mock_foo_obj.end_date,
                             self.mock_foo_obj.start_date +\
                             self.mock_foo_obj.duration)

    def test_end_date_argument_is_tried_to_set_to_a_time_before_start_date(
    self):
        """testing if end_date attribute will be updated to
        start_date + duration when the end_date argument is given as a value
        which is a date before start
        """

        self.kwargs["end_date"] = self.kwargs["start_date"] -\
    ...
    def test_end_date_attribute_is_tried_to_set_to_a_time_before_start_date(
    self):
        """testing if a the due attribute is re calculated from the start_date
        and duration attributes when the end_date is tried to be set to a date
        before start
        """

        new_end_date = self.mock_foo_obj.start_date -\
                       datetime.timedelta(days=10)

        self.mock_foo_obj.end_date = new_end_date

        self.assertEqual(self.mock_foo_obj.end_date,
                         self.mock_foo_obj.start_date +\
                         self.mock_foo_obj.duration)

    def test_end_date_attribute_is_shifted_when_start_date_passes_it(self):
        """testing if end_date attribute will be shifted when the start_date
        attribute passes it
        """

        time_delta = self.mock_foo_obj.end_date - self.mock_foo_obj.start_date
        self.mock_foo_obj.start_date += 2 * time_delta

        self.assertEqual(
            self.mock_foo_obj.end_date - self.mock_foo_obj.start_date,
            time_delta
        )

    def test_duration_argument_is_not_an_instance_of_timedelta_no_problem_if_start_date_and_end_date_is_present(
    self):
        ...
        end_date arguments are present.
        """

        test_values = [1, 1.2, "10", "10 days"]

        # no problem if there are start_date and end_date arguments
        for test_value in test_values:
            ...
            self.assertEqual(new_foo_obj.duration,
                             new_foo_obj.end_date - new_foo_obj.start_date)
    ...
    self):
        ...
        when end_date argument is also missing
        """

        test_values = [1, 1.2, "10", "10 days"]

        self.kwargs.pop("end_date")
    ...
    def test_duration_attribute_is_calculated_correctly(self):
        ...
        new_foo_entity.end_date = new_foo_entity.start_date +\
    ...
    def test_duration_attribute_is_set_to_not_an_instance_of_timedelta(self):
        ...
        for test_value in test_values:
            ...
            self.assertEqual(self.mock_foo_obj.duration,
                             self.mock_foo_obj.end_date -\
                             self.mock_foo_obj.start_date)

    def test_duration_attribute_expands_then_end_date_shifts(self):
        """testing if duration attribute is expanded then the end_date
        ...
        # end_date is postponed
        self.assertEqual(self.mock_foo_obj.end_date, start_date + new_duration)

    def test_duration_attribute_contracts_then_end_date_shifts_back(self):
        """testing if duration attribute is contracted then the end_date
    ...
    def test_init_start_date_and_end_date_argument_is_given(self):
        """testing if the attributes are initialized to:
    ...
    def test_init_end_date_and_duration_argument_is_given(self):
        """testing if the attributes are initialized to:
    ...
    def test_init_only_end_date_argument_is_given(self):
        """testing if the attributes are initialized to:
